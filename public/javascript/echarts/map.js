...
function focusNode(ec, payload) {
    ...
    model.eachSeries(function (seriesModel) {
        ...
        if (seriesModel.seriesIndex === payload.seriesIndex) {
            var mapModel=seriesModel;
            data = mapModel.getData();
            var dataIndex = payload.dataIndex;
            var el = data.getItemGraphicEl(dataIndex);
            if (!el) {
                return;
            }
            var dataType = payload.componentSubType;
            if (dataIndex !== null && dataType === 'effectScatter') {
                data.each(function (idx) {
                    var el = data.getItemGraphicEl(idx);
                    var symbol = el.childAt(0);
                    if (idx !== dataIndex) {
                        data.setItemVisual(idx,'color', "#888888");
                        el.updateData(data, idx);
